...
|---|---|---|---|---|---|
| Phase 0 | 持久化基础 | 无 | 未开始 | 设计评审通过 | Phase 0 DoD 全部完成 |
...
任务状态统一为：`[ ]` 未开始、`[-]` 进行中、`[x]` 已完成、`[!]` 阻塞、`[~]` 延期。当前所有任务必须保持 `[ ]`。
...
#### P0-T03：实现 SQLite connection 配置
...
#### P0-T04：实现 PRAGMA foreign_keys=ON 验证
...
#### P0-T09：验证 memory_records 和 memory_revisions 外键
...
#### P0-T10：验证 skills 和 skill_revisions 外键
...
#### P0-T11：验证 eval_packs 和 eval_runs 外键
...
#### P0-T13：测试 lock 竞争、续租、过期和 generation
...
#### P0-T15：测试 outbox 幂等和 superseded
...
#### P0-T16：测试 outbox retry_wait 和 dead_letter
...
#### P0-T20：测试 tombstone 和 archived 过滤
...
- 当前所有任务均为 `[ ] 未开始`；只有代码、测试和证据均完成后才可改为 `[x]`。
- 本次整理不实施代码、不创建数据库、不运行 Gateway、不创建 CLI、不执行 Phase 0。
